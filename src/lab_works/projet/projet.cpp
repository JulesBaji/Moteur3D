...
namespace M3D_ISICG
{
	const std::string Projet::_shaderFolder = "src/lab_works/projet/shaders/";
	...
	bool Projet::init()
	{
		...
		const std::string fragmentShaderStr = readFile( _shaderFolder + "mesh_texture_projet.frag" );
		const std::string vertexShaderStr	= readFile( _shaderFolder + "mesh_texture_projet.vert" );
		...
		// stockez-la comme attribut de la classe Projet.
	...
	void Projet::animate( const float p_deltaTime ) 
	{
	}
	...
	void Projet::handleEvents( const SDL_Event & p_event )
	{
